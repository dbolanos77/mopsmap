...
def call_mopsmap(wvl,size_equ,n,r_mod,sigma,r_min,r_max,m,nonabs_fraction,shape,num_theta):
  ...
  mopsmap_ref_index_file = open('tmp_mopsmap.m', 'w')
  
  # write refractive index file
  m=np.array(m,ndmin=1)
  mopsmap_input_file.write("refractive index file tmp_mopsmap.m \n")
  for i_m in range(m.shape[0]):
    mopsmap_ref_index_file.write('%10.8f \n'%m[i_m])
  mopsmap_ref_index_file.close()
  ...
  if isinstance(wvl, basestring):
    wvl=[wvl,]
  if n.shape[0]>1 and len(wvl)==1:
